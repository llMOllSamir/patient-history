...
import { useUpdateCervix } from '../../hooks/cervical'

export default function CervicalUpdate() {
    const { data: patient } = useSelector(state => state.patient)
    const { cervical } = useSelector(state => state.cervical)
    ...
    const { mutate, isLoading, isError } = useUpdateCervix({ onError, onSuccess })
    ...
    const formik = useFormik({
        initialValues: {
            ...
            "hpv_vaccine": cervical?.hpv_vaccine === 1 ? "yes" : "no",
            "via_test_result": cervical?.via_test_result,
            "via_test_comment": cervical?.via_test_comment,
            "pap_smear_result": cervical?.pap_smear_result,
            "pap_smear_comment": cervical?.pap_smear_comment,
            "recommendations": cervical?.recommendations
        },
        validationSchema
        ,
        onSubmit: (values) => {
                ...
                <div>
                    ...
                    <div className='border-2 border-gray-400 flex flex-col gap-3  lg:w-3/4 my-3 xl:w-1/2 rounded-xl p-4'>
                        <label htmlFor="hpv_vaccineYes" className='flex gap-3 font-medium'>
                            <input checked={formik.values.hpv_vaccine === "yes"} type="radio" className='checkedInput' onChange={formik.handleChange} onBlur={formik.handleBlur} value={"yes"} name="hpv_vaccine" id="hpv_vaccineYes" />
                            Yes
                        </label>
                        <label htmlFor="hpv_vaccineNo" className='flex gap-3 font-medium'>
                            <input checked={formik.values.hpv_vaccine === "no"} type="radio" className='checkedInput' onChange={formik.handleChange} onBlur={formik.handleBlur} value={"no"} name="hpv_vaccine" id="hpv_vaccineNo" />
                            No
                        </label>
                    </div>
