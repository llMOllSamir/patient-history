...
export default function AddDoctor() {
  ...
  const formik = useFormik({
    ...
    validateOnBlur: true,
    onSubmit: (values) => {
      id ? dispatch(updateDoctor({ id, values })) : dispatch(addDoctor(values));
    },
  });
  useEffect(() => {
    ...
    <div id="add_doctor_cont">
      ...
      <div className="flex flex-wrap flex-col lg:m-20">
        ...
        >
          <div
            id="input-cont"
            className="flex flex-col items-center md:flex-row flex-wrap"
          >
          ...
          {inputTextArr.map((e, i) =>
            formik.errors[e.id] && formik.touched[e.id] ? (
              <p key={i} className="text-center text-red-600 text-sm mt-3">
                {formik.errors[e.id]}
              </p>
            ) : null
          )}
