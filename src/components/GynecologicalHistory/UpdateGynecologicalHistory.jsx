...
import { useUpdateGynecologicalHistory } from '../../hooks/gynecologicalHistory';
import { ImSpinner6 } from 'react-icons/im';
import { useFormik } from "formik";
import * as yup from "yup";

export default function UpdateGynecologicalHistory() {
  const { user } = useSelector(state => state.auth)
  const navigate = useNavigate()
  const { data: patient } = useSelector(state => state.patient)
  const { gynecologicalHistory } = useSelector(state => state.gynecologicalHistory)
  const onSuccess = () => { navigate(`/patient/gynecological-history/${patient?.id}`) }
  const { isLoading, mutate } = useUpdateGynecologicalHistory({ id: patient?.id, onSuccess })
  ...
  const formik = useFormik({
    initialValues: {
      ...
      date_of_last_period: gynecologicalHistory.date_of_last_period || "",
      menstrual_cycle_abnormalities: gynecologicalHistory.menstrual_cycle_abnormalities || "",
      contact_bleeding: gynecologicalHistory.contact_bleeding === true ? "true" : "false" || "",
      menopause: gynecologicalHistory.menopause === true ? "true" : "false" || "",
      menopause_age: gynecologicalHistory.menopause_age || "",
      using_of_contraception: gynecologicalHistory.using_of_contraception === true ? "true" : "false" || "",
      contraception_method: gynecologicalHistory.contraception_method || "",
      other_contraception_method: gynecologicalHistory.other_contraception_method || ""
    },
    validationSchema,
    onSubmit: (values) => {
      const data = {
  ...
  useEffect(() => {
    console.log(gynecologicalHistory);
