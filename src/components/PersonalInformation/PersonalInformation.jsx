...
export default function PersonalInformation() {
  ...
  const { isLoading, error, isError } = useGetPatient({ code: id, onSuccess })
  ...
  if (data) return <React.Fragment>
    <div className='mx-4 lg:mx-16  grid  grid-cols-1 md:grid-cols-2  gap-5 '>
      ...
      <ArticalInfo description={data.date_of_birth} title={"Date of birth"} />

      <ArticalInfo description={data.age} title={"age"} />

      <ArticalInfo description={data.marital_state} title={"Marital status"} />
      ...
      <ArticalInfo description={data.phone_number} phone={true} title={"Phone number"} />

      <ArticalInfo description={data.email || "Not Found"} title={"E-mail"} />

      <ArticalInfo description={data.relative_phone} phone={true} title={"Home number"} />
      <ArticalInfo description={data.relative_name} title={"Relative name"} />
    </div>
    <div className='flex print:hidden gap-x-8 gap-y-4 justify-end md:flex-row flex-col my-10  items-end md:items-center me-16'>
      <Link to={`/patient/personal-information/update-patient/${data.patient_code}`} className='rounded-lg text-white bg-blue-700 flex gap-4 px-10 py-2'><MdEdit />Edit</Link>
      <button className='rounded-lg text-white bg-fuchsia-900 flex gap-4 px-10 py-2' onClick={() => { window.print() }}><FiDownload />Download a copy</button>
    </div>
  </React.Fragment>
...
const ArticalInfo = ({ title, description, col, phone = null }) => {
  ...
  return <article className={`flex flex-col font-medium capitalize ${col && `md:col-span-${col}`}`}>
    <h3 className=' text-black   text-xl'>{title || ""}</h3>
    {phone ? <a className=' text-gray-600 text-base w-fit' href={`tel:${description}`} >{description || ""}</a>
