...
import { setDoctorsData, setHistory } from "../../store/slices/doctorSlice";
import LoadingPatient from "../../LoadingPatient";
import { useGetDoctorsAndAdminActivities } from "../../hooks/doctor";
...
function Dashboard() {
  ...
  const doctorsAndAdminActivities = useGetDoctorsAndAdminActivities({
    id,
  });

  // Accessing results
  const doctorsResult = doctorsAndAdminActivities[0];
  const adminActivitiesResult = doctorsAndAdminActivities[1];

  React.useEffect(() => {
    if (
      doctorsResult.status === "success" &&
      adminActivitiesResult.status === "success"
    ) {
      const doctorsData = doctorsResult.data.data;
      const adminActivitiesData = adminActivitiesResult.data.data;
      dispatch(setDoctorsData(doctorsData));
      dispatch(setHistory(adminActivitiesData));
    }
  }, [adminActivitiesResult, dispatch, doctorsResult]);

  // Handle loading, error, and data for both queries
  if (doctorsResult.isLoading || adminActivitiesResult.isLoading) {
    return <LoadingPatient />;
  }

  if (doctorsResult.error || adminActivitiesResult.error) {
    return (
      <div className="mx-4  text-center h-1/2 items-center justify-center   flex flex-col gap-5  ">
        <h2 className="font-bold text-red-500 text-3xl">
          Error: {doctorsResult.error || adminActivitiesResult.error}
        </h2>
      </div>
    );
  }
...
function DoctorItem({ doctor }) {
  return (
    <div className="flex flex-col justify-center px-5 py-4 w-full border-r border-b border-l border-solid bg-neutral-200 border-stone-300 max-md:max-w-full">
      <div className="flex gap-5 justify-between w-full max-md:flex-wrap max-md:max-w-full">
        ...
        <Link className="flex gap-1 px-2.5 py-0.5 text-center whitespace-nowrap bg-rose-500 rounded-lg hover:bg-rose-600">
